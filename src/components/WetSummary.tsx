  ...
  return (
    <div className="max-w-5xl mx-auto py-4 sm:py-8 px-3 sm:px-4">
      <div className="text-center mb-4 sm:mb-6">
        <p className="text-[10px] sm:text-xs font-medium uppercase tracking-widest text-gray-500 mb-1">Report</p>
        <h2 className="text-2xl sm:text-3xl font-semibold text-gray-50 mb-1">Wet Weight Summary</h2>
      ...
      {/* Summary Table */}
      <div className="bg-base-900 border border-base-700 rounded-lg overflow-x-auto mb-4 sm:mb-6">
        <table className="w-full text-left text-xs sm:text-sm min-w-[480px]">
          <thead>
            <tr className="bg-base-800 border-b border-base-700">
              <th className="px-2 sm:px-3 py-2 sm:py-2.5 text-[10px] sm:text-xs font-medium uppercase tracking-widest text-gray-500">Strain</th>
              <th className="px-2 py-2 sm:py-2.5 text-[10px] sm:text-xs font-medium uppercase tracking-widest text-gray-500 text-right">Exp</th>
              <th className="px-2 py-2 sm:py-2.5 text-[10px] sm:text-xs font-medium uppercase tracking-widest text-gray-500 text-right">Done</th>
              <th className="px-2 py-2 sm:py-2.5 text-[10px] sm:text-xs font-medium uppercase tracking-widest text-gray-500 text-right">Total (g)</th>
              <th className="px-2 py-2 sm:py-2.5 text-[10px] sm:text-xs font-medium uppercase tracking-widest text-gray-500 text-right">LBS</th>
              <th className="px-2 sm:px-3 py-2 sm:py-2.5 text-[10px] sm:text-xs font-medium uppercase tracking-widest text-gray-500 text-right">Avg</th>
            </tr>
          </thead>
          <tbody>
            {summaries.map((s, i) => (
              <tr key={i} className={`border-b border-base-700/50 ${i % 2 === 0 ? 'bg-base-900' : 'bg-base-800/30'}`}>
                <td className="px-2 sm:px-3 py-2 sm:py-2.5 font-medium text-gray-100">{s.strain}</td>
                <td className="px-2 py-2 sm:py-2.5 text-right font-mono text-gray-400">{s.plantCount}</td>
                <td className={`px-2 py-2 sm:py-2.5 text-right font-mono ${
                  s.plantsWeighed === s.plantCount ? 'text-green-400' : 'text-amber-400'
                }`}>{s.plantsWeighed}</td>
                <td className="px-2 py-2 sm:py-2.5 text-right font-mono text-gray-300">{s.totalGrams.toFixed(1)}</td>
                <td className="px-2 py-2 sm:py-2.5 text-right font-mono text-gray-300">{s.totalLbs.toFixed(2)}</td>
                <td className="px-2 sm:px-3 py-2 sm:py-2.5 text-right font-mono text-gray-400">{s.avgPerPlant.toFixed(1)}</td>
              </tr>
            ))}
            {/* Grand Total */}
            <tr className="bg-base-800 border-t-2 border-base-600">
              <td className="px-2 sm:px-3 py-2 sm:py-2.5 font-semibold text-gray-100">TOTAL</td>
              <td className="px-2 py-2 sm:py-2.5 text-right font-mono font-semibold text-gray-100">{grandPlants}</td>
              <td className="px-2 py-2 sm:py-2.5 text-right font-mono font-semibold text-gray-100">{grandWeighed}</td>
              <td className="px-2 py-2 sm:py-2.5 text-right font-mono font-semibold text-gray-100">{grandGrams.toFixed(1)}</td>
              <td className="px-2 py-2 sm:py-2.5 text-right font-mono font-semibold text-gray-100">{grandLbs.toFixed(2)}</td>
              <td className="px-2 sm:px-3 py-2 sm:py-2.5 text-right font-mono font-semibold text-gray-300">{grandAvg.toFixed(1)}</td>
            </tr>
          </tbody>
        </table>
      </div>
              ...
              <table className="w-full text-left text-xs sm:text-sm">
                <thead className="sticky top-0 bg-base-900 border-b border-base-700">
                  <tr>
                    ...
                    <th className="px-2 sm:px-4 py-1 sm:py-1.5 text-[10px] sm:text-xs font-medium uppercase tracking-widest text-gray-500 text-right">Weight (g)</th>
                    <th className="hidden sm:table-cell px-4 py-1.5 text-xs font-medium uppercase tracking-widest text-gray-500">Time</th>
                  </tr>
                </thead>
                <tbody>
                  {strainReadings.map((r, i) => (
                    <tr key={r.id} className={i % 2 === 0 ? 'bg-base-900' : 'bg-base-800/30'}>
                      ...
                      <td className="px-2 sm:px-4 py-1 sm:py-1.5 font-mono text-green-400/80 truncate max-w-[140px] sm:max-w-none">{r.tagId}</td>
                      <td className="px-2 sm:px-4 py-1 sm:py-1.5 font-mono text-right text-gray-200">{r.weightGrams.toFixed(1)}</td>
                      <td className="hidden sm:table-cell px-4 py-1.5 text-gray-500 text-xs">
      ...
      <div className="flex gap-3 mt-4 sm:mt-6">
        <button
          onClick={onExport}
          className="flex-1 py-3 bg-green-600 hover:bg-green-500 text-white font-medium rounded-lg transition-colors border border-green-500/30 text-sm sm:text-base"
        >
          Export Excel
        </button>
        <button
